...
const server = http.createServer((req, res) => {
 ...
 if (req.url === '/products' && req.method === 'POST') {
  ...
  if (req.url === '/products' && req.method === 'GET') {
    sendJSON(res, db.prepare('SELECT id, name, price, category, description, image FROM products ORDER BY id').all());
    return;
  }
    ...
    req.on('end', () => {
      try {
        ...
        const result = db.prepare(
          'UPDATE products SET name = ?, price = ?, category = COALESCE(?, category), description = COALESCE(?, description), image = COALESCE(?, image) WHERE id = ?'
        ).run(
          data.name,
          Number(data.price),
          data.category,
